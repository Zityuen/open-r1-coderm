...
which requires transformers, torch, etc.):
    ...
    extract_code
    _run_unittest_with_per_test_metrics   →  single-solution per-test dict
    _compute_bik_matrices                 →  B_correct/B_wrong split matrices
    _default_eq10_aggregation             →  Equation 10 reward (numpy)
    cross_solution_unittest_reward        →  end-to-end with optional eval aggregation
...
def _run_unittest_with_per_test_metrics(
    ...
    """Execute the full unittest suite once for a single solution and return
    a per-test pass/fail map.  Returns dict mapping test_id → 0 or 1."""
...
def _default_eq10_aggregation(
    ...
    """Equation 10 aggregation (numpy vectorised)."""
    ...
    B_correct = B[correct_mask]
    B_wrong = B[~correct_mask]
    M_plus = B_correct.shape[0]
    M_minus = B_wrong.shape[0]

    prod_correct = np.prod(B_correct, axis=0)
    sum_correct = np.sum(B_correct, axis=0)
    prod_wrong = np.prod(B_wrong, axis=0)
    sum_wrong = np.sum(B_wrong, axis=0)
...
def cross_solution_unittest_reward(
    ...
    """End-to-end reward using _compute_bik_matrices + optional eval aggregation."""
...
class TestRunUnittestWithPerTestMetrics(unittest.TestCase):
    """Test _run_unittest_with_per_test_metrics (single-solution runner)."""

    def test_correct_solution_all_pass(self):
        metrics = _run_unittest_with_per_test_metrics(TEST_CODE, CORRECT_SOLUTION)
    ...
    def test_wrong_solution_all_fail(self):
        metrics = _run_unittest_with_per_test_metrics(TEST_CODE, WRONG_SOLUTION_OFF_BY_ONE)
        self.assertEqual(len(metrics), 3)
        self.assertTrue(all(v == 0 for v in metrics.values()),
                        f"All tests should fail: {metrics}")

    def test_partial_pass(self):
        metrics = _run_unittest_with_per_test_metrics(TEST_CODE, WRONG_SOLUTION_SUBTRACT)
        self.assertEqual(len(metrics), 3)
    ...
    def test_empty_inputs(self):
        self.assertEqual(_run_unittest_with_per_test_metrics("", CORRECT_SOLUTION), {})
        self.assertEqual(_run_unittest_with_per_test_metrics(TEST_CODE, ""), {})
        self.assertEqual(_run_unittest_with_per_test_metrics("", ""), {})


class TestComputeBikMatrices(unittest.TestCase):
    """Test _compute_bik_matrices (multi-solution split-matrix builder)."""

    def test_shape(self):
        test_ids, B_correct, B_wrong = _compute_bik_matrices(TEST_CODE, SOLUTIONS)
        self.assertEqual(len(test_ids), 3, "3 test methods")
        self.assertEqual(len(B_correct), 3, "K=3 rows in B_correct")
        self.assertEqual(len(B_wrong), 3, "K=3 rows in B_wrong")
        # 1 correct, 2 wrong
        for row in B_correct:
            self.assertEqual(len(row), 1, "M+=1 correct solution")
        for row in B_wrong:
            self.assertEqual(len(row), 2, "M-=2 wrong solutions")

    def test_correct_passes_all(self):
        test_ids, B_correct, B_wrong = _compute_bik_matrices(TEST_CODE, SOLUTIONS)
        # Correct solution passes all 3 tests
        for k, row in enumerate(B_correct):
            self.assertEqual(row[0], 1.0, f"Correct solution should pass test {k}")

    def test_wrong_off_by_one_fails_all(self):
        test_ids, B_correct, B_wrong = _compute_bik_matrices(TEST_CODE, SOLUTIONS)
        # Wrong off-by-one (index 0 in wrong) fails all tests
        for k, row in enumerate(B_wrong):
            self.assertEqual(row[0], 0.0, f"Off-by-one should fail test {k}")

    def test_wrong_subtract_partial(self):
        test_ids, B_correct, B_wrong = _compute_bik_matrices(TEST_CODE, SOLUTIONS)
        # Wrong subtract (index 1 in wrong) passes exactly 1 test (test_add_zero)
        pass_count = sum(row[1] for row in B_wrong)
        self.assertEqual(pass_count, 1.0, "Subtract solution passes exactly 1 test")

    def test_empty_inputs(self):
        test_ids, B_correct, B_wrong = _compute_bik_matrices("", SOLUTIONS)
        self.assertEqual(test_ids, [])
        self.assertEqual(B_correct, [])
        self.assertEqual(B_wrong, [])

    def test_empty_solutions(self):
        test_ids, B_correct, B_wrong = _compute_bik_matrices(TEST_CODE, [])
        self.assertEqual(test_ids, [])
