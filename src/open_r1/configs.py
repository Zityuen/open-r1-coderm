...
class GRPOScriptArguments(ScriptArguments):
    ...
    unittest_reward_aggregation: Optional[str] = field(
        ...
        metadata={
            ...
            "cross_solution_unittest_reward. Available variables: B (list[list[int]] — B[i][k]=1 if solution i "
            "passes test k), correct_mask (list[bool]), M_plus (int), M_minus (int), K (int), math module, "
            "and common builtins (sum, len, min, max, range, zip, enumerate, all, any, abs, float, int). "
            "If None (default), uses the built-in Equation 10 aggregation."
        },
    )